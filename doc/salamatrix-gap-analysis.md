...
| Manifest/package | MVP | Strict UTF-8 JSON parsing validates schema 1, package/runtime identity and minimum version, safe entry point, package-owned SVG `icon`/`iconDark` assets, capabilities, and up to 64 command records; Ask-AI can save a validated package and refresh discovery. Missing separately installed runtime adapters are represented explicitly during discovery instead of failing silently. | Dependencies, locales, settings declarations, installed-state management, uninstall, and richer diagnostics. |
