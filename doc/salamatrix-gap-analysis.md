...
- Modern manifest-backed scripts run through the shared worker bootstrap;
  ...
  scripts still cannot remain alive after their execution ends. If a required
  separately installed runtime is missing, discovery keeps the extension in a
  visible waiting-for-runtime state and reactivates it after the runtime appears.
- The strict schema-1 manifest parser validates package/runtime identity,
  ...
  and richer handler lifecycle metadata are still missing.
...
| Manifest/package | MVP | Strict UTF-8 JSON parsing validates schema 1, package/runtime identity and minimum version, safe entry point, package-owned SVG `icon`/`iconDark` assets, capabilities, typed settings declarations/defaults, and up to 64 command records; persistent packages publish up to 16 declared commands to the native menu/toolbar surface and pass each selected command's declared handler to the worker; Ask-AI can save a validated package and refresh discovery. Missing separately installed runtime adapters are represented explicitly during discovery instead of failing silently. | Dependencies, locales, settings migrations/files, handler lifecycle metadata, installed-state management, uninstall, and richer diagnostics. |
