...
| Extension management | Partial | The existing Plugin Manager remains the single management surface: it shows ordinary `.SPL` plugins and now appends discovered manifest-extension rows from `Salamatrix.Extensions` with name, state, version, runtime, entry point, and manifest ID details. Manifest rows are informational and are not treated as loadable `.SPL` files. No separate Extension Manager is planned. | Scripted-extension installed-state model, enable/disable/configure/remove, dependency prompts, and richer runtime/status details in the existing Plugin Manager. |
