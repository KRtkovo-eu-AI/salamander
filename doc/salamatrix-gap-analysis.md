...
| Runtime adapters | MVP | `Salamatrix.Runtime` registers/enumerates versioned adapters and executes manifest entry points through structured requests/results. Automation currently owns legacy ActiveScript plus optional out-of-process CPython, PowerShell, and PHP CLI adapters; the broker contract is independent of Automation so separate runtime `.spl` providers can register alongside it. | Move modern providers out of Automation, cancellation beyond process termination, debugging, dependency environments, and separately installable runtime components. |
| JavaScript runtime | Worker bootstrap only | Windows JScript remains the compatibility fallback for `.js`; a dependency-free Node.js SMX1 worker bootstrap exists as the basis of a future standalone JavaScript runtime provider. It is intentionally not registered by Automation. | Register/discover a standalone JavaScript runtime `.spl`, define `.js`/`.mjs` ownership and compatibility fallback, and provide package deployment policy. |
...
the same service by native callers and workers. The current CPython,
PowerShell, and PHP CLI implementations are still owned by
`CAutomationSalamatrixBridge`, so they are not yet independently installable
`.spl` providers. The target packaging is one provider plugin per language (or
an explicitly grouped runtime package), with interpreter discovery and worker
bootstrap beside that provider; Automation retains only its legacy ActiveScript
