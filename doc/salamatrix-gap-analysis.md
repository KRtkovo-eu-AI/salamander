...
| AI automation assistant | Partial | `Salamatrix.AI` exposes provider registration, structured generation validation, parsed effect flags, runtime/existing-script/feedback hints, a safe `generate`/`preview` worker seam, and Automation's optional bounded local command provider selected by `SALAMATRIX_AI_COMMAND`; Ask-AI gathers source-panel context, offers a bounded three-iteration repair loop, previews/copies the result, offers explicit Run/Save As, and can save a strict manifest-validated extension package. | Richer conversation history/editing, bundled llama.cpp inference, and optional remote providers. |
