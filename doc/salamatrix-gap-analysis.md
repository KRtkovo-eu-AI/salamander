...
| Lua runtime | Process MVP | `Lua` discovers `SALAMATRIX_LUA`, `lua.exe`, `lua55.exe`, or `lua54.exe`, runs `.lua` entries out of process, and ships a dependency-free Lua SMX1 worker with the common Salamander facade. `LuaRuntime.SPL`, its worker, installer/catalog entries, and a visible demo package pass Debug/Release x64 builds, PE/package verification, Lua 5.4.8 syntax checks, and an isolated storage/notification SMX1 call sequence. | Direct provider load through Salamander, wider interpreter-version matrix, debugger integration, and dependency environment policy. |
