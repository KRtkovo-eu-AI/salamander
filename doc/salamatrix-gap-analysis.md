...
| Events | MVP | `Salamatrix.Events` maps host lifecycle/settings/configuration/color/panel events and successful shared-Sides path/selection/tab/refresh operations to unsubscribe-safe native callbacks; Automation exposes `subscribe/unsubscribe` with copied payloads. Persistent worker sessions now enqueue bounded event frames instead of writing from the core callback directly into a potentially back-pressured pipe. | Persistent extension instances, UI-thread marshalling for richer event payloads, coalescing, event replay, user-driven side-change hooks in the legacy core ABI, and unload-safe leases across modern runtimes. |
...
3. Event service with a small first set: startup/shutdown, active side/tab,
   ...
   settings/configuration/colors, panel swap, active-panel events, and
   successful shared-Sides path/selection/tab/refresh operations; persistent
   instance queues now use bounded worker-session frames. User-driven side
   changes and richer tab lifecycle hooks remain.**
