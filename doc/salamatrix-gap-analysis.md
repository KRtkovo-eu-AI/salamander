...
- The shared native UI service exposes only progress. The older
...
- There is no AI assistant, model-provider API, machine-readable Automation API
  schema, capability/effect model, static validator, preview/run/save workflow,
  or local inference integration.
...
| Extension command registration | Partial | Discovery-time metadata gives one script a stable id, caption, placement hints, and context requirements. | Multiple commands/handlers, runtime registration, enable/visible callbacks, icons, toolbar/hotkey binding, command ownership and unload. |
...
| Testing | Initial | Standalone `/W4 /WX` tests cover the strict manifest parser, Storage persistence, Events subscribe/publish/self-unsubscribe/capacity/payload validation, Extensions registration/lifecycle/ownership, and an integration test for Python/PowerShell/PHP process execution, output capture, and timeout. | Core registry and runtime contract tests, UI tests, extension fixtures, lifecycle/unload tests, and end-to-end Salamander API adapter tests. |
