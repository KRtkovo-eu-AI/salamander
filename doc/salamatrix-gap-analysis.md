...
| Events | MVP | `Salamatrix.Events` maps host lifecycle/settings/configuration/color/panel events to unsubscribe-safe native callbacks; Automation exposes `subscribe/unsubscribe` with copied payloads. Persistent worker sessions now enqueue bounded event frames instead of writing from the core callback directly into a potentially back-pressured pipe. | Persistent extension instances, UI-thread marshalling for richer event payloads, coalescing, event replay, and unload-safe leases across modern runtimes. |
