...
- The shared native UI service exposes progress plus a bounded native input-box
  host call for modern workers. The older
  Automation-only Forms implementation has Form, Label, TextBox, CheckBox, and
  Button, but it has not been moved under `Salamatrix.UI`. ComboBox and ListView
  are enum placeholders only.
- `Salamatrix.AI` now has a provider-neutral contract and Automation registers
  ...
  The assistant UI, API-context selection, capability/effect model, validator,
  preview/run/save workflow, and a bundled inference backend are still missing.
...
| Existing Salamander commands | MVP | Four aliases route to `SALCMD_QUICKRENAME`, `SALCMD_COPY`, and `SALCMD_MOVE`. | Enumerated stable catalog, parameters, synchronous/modal results, more commands, state/change notifications. |
| Programmatic file operations | Missing | Only interactive command posting exists. | Copy/move/rename/delete/create-directory APIs with typed options, progress, cancellation, and final results. |
| Extension command registration | Partial | Discovery-time metadata gives one script a stable id, caption, placement hints, and context requirements; a persistent worker can now register/unregister one owner-scoped command and trigger a menu rebuild. | Multiple commands/handlers, enable/visible callbacks, icons, toolbar/hotkey binding, and richer ownership/unload leases. |
...
| Shared UI framework | Partial | `Salamatrix.UI` now provides a reusable native `IDialog`/`IControl` model and `NativeDialog` implementation for labels, text boxes, check/radio buttons, combo boxes, and buttons; the worker `ui.inputBox` call uses that service. | Pickers, ListView/TreeView data binding, TabControl, notifications, validation/events, layout, DPI/theme/accessibility, and migration of the legacy Automation Forms wrappers. |
...
| AI automation assistant | Partial | `Salamatrix.AI` exposes provider registration, structured generation validation, and parsed effect flags; Automation supplies an optional bounded local command provider selected by `SALAMATRIX_AI_COMMAND`. | API-context selection, generation UI, repair loop, effect preview, run/save flow, bundled llama.cpp inference, and optional remote providers. |
...
Move the native control implementation under `Salamatrix.UI`, then keep the old
Automation Forms classes as compatibility wrappers. Build controls in this
order: dialog/layout, Label, TextBox, CheckBox, Button, ComboBox, pickers,
ListView, TreeView, RadioButton, TabControl, notifications. All controls need
