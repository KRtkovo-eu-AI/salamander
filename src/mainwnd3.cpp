...
bool CMainWindow::EnsurePanelWindowCreated(CFilesWindow* panel)
{
    ...
    panel->ApplyPendingSettings();
    UpdatePanelTabVisibility(panel->GetPanelSide());
    return true;
}
...
void CMainWindow::SwitchPanelTab(CFilesWindow* panel)
{
    ...
    bool windowReady = EnsurePanelWindowCreated(panel);
    bool canFocusNow = (Created && panel->HWindow != NULL);
    if (!canFocusNow && Created && windowReady && panel->HWindow != NULL)
        canFocusNow = true;
    ...
    if (panel->HWindow != NULL)
        panel->ApplyDeferredConfigPath(this);
...
CFilesWindow* CMainWindow::CreateDuplicatePanelTab(CPanelSide targetSide, CFilesWindow* sourcePanel, int insertIndex)
{
    ...
    if (newPanel->HWindow == NULL && !EnsurePanelWindowCreated(newPanel))
    {
        TRACE_E("Unable to create panel window while duplicating tab");
        ClosePanelTab(newPanel, false);
...
bool CMainWindow::CommandReopenClosedTab(CPanelSide side)
{
    ...
    {
        panel->StatusLineVisible = entry.StatusLineVisible ? TRUE : FALSE;
    ...
    {
        panel->DirectoryLineVisible = entry.DirectoryLineVisible ? TRUE : FALSE;
    ...
    {
        panel->HeaderLineVisible = entry.HeaderLineVisible ? TRUE : FALSE;
...
*/
        ...
        if (leftPanel->HWindow == NULL && !EnsurePanelWindowCreated(leftPanel))
        {
        ...
        if (rightPanel->HWindow == NULL && !EnsurePanelWindowCreated(rightPanel))
        {
