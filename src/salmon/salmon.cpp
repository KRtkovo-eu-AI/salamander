...
HINSTANCE LoadSLG(const char* slgName)
{
    ...
    if (hSLG == NULL)
        SalmonMessageBox(NULL, "Internal error: cannot load any language file. Please report this at https://samandarin.net/issues.", APP_NAME, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
    return hSLG;
}
...
BOOL IsTestCommandLine(const char* cmdLine)
{
    while (*cmdLine == ' ' || *cmdLine == '\t')
        cmdLine++;
    return _stricmp(cmdLine, "--test") == 0 || _stricmp(cmdLine, "/test") == 0;
}

BOOL WriteTestReportFile(const char* fileName, const char* text)
{
...
int RunTestCompression(const char* slgName)
{
    HINSTANCE hLanguage = LoadSLG(slgName);
    if (hLanguage != NULL)
        HLanguage = hLanguage;

    char testDir[SAL_MAX_PATH];
    DWORD testDirLen = GetTempPath(SAL_MAX_PATH, testDir);
    if (testDirLen == 0 || testDirLen >= SAL_MAX_PATH)
    {
        SalmonMessageBox(NULL, "Cannot get the temporary directory for the test bug report.", APP_NAME, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
        return SALMON_RET_ERROR;
    }
    lstrcpyn(testDir + strlen(testDir), "OpenSalamanderBugReporterTest\\", SAL_MAX_PATH - (int)strlen(testDir));

    if (SHCreateDirectoryEx(NULL, testDir, NULL) != ERROR_SUCCESS && !DirExists(testDir))
    {
        SalmonMessageBox(NULL, "Cannot create the test bug report directory.", APP_NAME, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
        return SALMON_RET_ERROR;
    }

    lstrcpyn(BugReportPath, testDir, MAX_PATH);

    SYSTEMTIME lt;
    GetLocalTime(&lt);

    char reportName[MAX_PATH];
    sprintf(reportName, "TEST-BugReporter-%04u%02u%02u-%02u%02u%02u",
            lt.wYear, lt.wMonth, lt.wDay, lt.wHour, lt.wMinute, lt.wSecond);

    char txtPath[SAL_MAX_PATH];
    sprintf(txtPath, "%s%s.TXT", BugReportPath, reportName);
    if (!WriteTestReportFile(txtPath, "Open Salamander Bug Reporter test report.\r\nThis file was generated by salmon.exe --test.\r\n"))
    {
        SalmonMessageBox(NULL, "Cannot write the test bug report file.", APP_NAME, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
        return SALMON_RET_ERROR;
    }
    ...
    BugReports.DestroyMembers();
    CBugReport item;
    lstrcpyn(item.Name, reportName, MAX_PATH);
    BugReports.Add(item);
    ReportOldBugs = FALSE;

    CCompressParams compressParams;
    ZeroMemory(&compressParams, sizeof(compressParams));
    BOOL ret = CompresBugReports(&compressParams);

    char msg[2 * MAX_PATH];
    if (ret)
    {
        sprintf(msg, "Test bug report was compressed successfully.\n\nFolder:\n%s", BugReportPath);
        SalmonMessageBox(NULL, msg, APP_NAME, MB_OK | MB_ICONINFORMATION | MB_SETFOREGROUND);
        OpenFolder(NULL, BugReportPath);
        return SALMON_RET_OK;
    }

    sprintf(msg, "Test bug report compression failed.\n\nReason:\n%s\n\nFolder:\n%s", compressParams.ErrorMessage, BugReportPath);
    SalmonMessageBox(NULL, msg, APP_NAME, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
    OpenFolder(NULL, BugReportPath);
    return SALMON_RET_ERROR;
}
