...
public:
    ...
    static const char* GetStaticApiDescription()
    {
        return
            ...
            "\"Salamander.fileSystem\":{\"methods\":[\"addItem\",\"addItems\"],\"itemFields\":[\"id\",\"name\",\"compactName\",\"icon\",\"iconDark\",\"fileIcon\",\"directory\",\"enabled\",\"columns\"],\"compactNameSemantics\":\"Optional alternate item name displayed outside Detailed view.\",\"fileIconSemantics\":\"A fully qualified Unicode file path whose embedded icon resource is displayed without consulting file-extension associations.\",\"batchSemantics\":\"Use addItems for a complete listing snapshot to avoid one synchronous worker-host round-trip per row.\"},"
            "\"Salamander.ui\":{\"methods\":[\"messageBox\",\"fileProperties\",\"inputBox\",\"notify\",\"controls\",\"uptime\",\"pickFile\",\"pickFolder\",\"progress\",\"progress.update\",\"progress.step\",\"progress.setTotals\",\"progress.setPositions\",\"progress.setTitle\",\"progress.setCancelEnabled\",\"progress.cancelled\",\"progress.close\",\"dialog\",\"dialog.add\",\"dialog.get\",\"dialog.set\",\"dialog.validation\",\"dialog.events\",\"dialog.item\",\"dialog.column\",\"dialog.selection\",\"dialog.clearItems\",\"setValidation\",\"onChange\",\"offChange\",\"addColumn\",\"setSelectedIndex\"],\"fileProperties\":{\"arguments\":[\"fullyQualifiedPath\"],\"resultFields\":[\"shown\",\"error\"],\"ownership\":\"The Salamander host invokes the native Windows File Properties sheet.\"},\"controlKinds\":[\"label\",\"statictext\",\"textbox\",\"checkbox\",\"radio\",\"combobox\",\"button\",\"listview\",\"treeview\",\"tabcontrol\",\"folderpicker\",\"filepicker\",\"groupbox\",\"hyperlink\",\"progressbar\",\"arrowbutton\",\"textarrowbutton\",\"colorarrowbutton\",\"toolbarheader\"],\"progressStyles\":[1,2],\"layout\":true,\"validation\":true,\"events\":true,\"selection\":true,\"notifications\":true,\"controlsShowcase\":true},"
            ...
            "\"Salamatrix.extensionManifest\":{\"schema\":2,\"schemaVersionAlias\":\"schemaVersion\",\"compatibleSchemaVersions\":[1,2],\"fields\":[\"id\",\"name\",\"version\",\"description\",\"runtime\",\"entryPoint\",\"web\",\"security\",\"capabilities\",\"commands\",\"viewers\",\"fileSystems\"],\"commandFields\":[\"id\",\"title\",\"handler\",\"path\",\"menu\",\"placement\",\"contextMenu\",\"toolbar\",\"toolbarMenu\",\"requires\"],\"fileSystemFields\":[\"id\",\"name\",\"listHandler\",\"openHandler\",\"icon\",\"iconDark\",\"defaultFileIcon\",\"refreshIntervalMs\",\"refreshDepth\",\"refreshPaths\",\"rootItems\",\"columns\",\"actions\"],\"fileSystemRootItemsSemantics\":\"Optional directory-only items rendered synchronously at the provider root without starting a runtime worker.\",\"commandPathSemantics\":\"A path-only command changes the active panel directly without starting a runtime worker, requires panels.write, and cannot also declare handler.\"},"
...
public:
    ...
    virtual const char* WINAPI GetApiDescriptionSlice(const char* topic) const
    {
        ...
        if (strcmp(topic, "execution") == 0 || strcmp(topic, "commandContext") == 0)
            return "{\"version\":\"1.2\",\"topic\":\"execution\",\"objects\":{\"Salamander\":{\"invocationFieldsByRuntime\":{\"JavaScript.Node\":[\"commandId\",\"commandHandler\",\"invocation\"],\"Python.CPython\":[\"command_id\",\"command_handler\",\"invocation\"],\"PowerShell\":[\"CommandId\",\"CommandHandler\",\"invocation\"],\"PHP.CLI\":[\"command_id\",\"command_handler\",\"invocation\"],\"Lua\":[\"command_id\",\"command_handler\",\"invocation\"]},\"semantics\":\"The extension entry point executes at top level. Viewer and file-system roles receive a parsed invocation object.\"}}}";
        if (strcmp(topic, "extensions") == 0 || strcmp(topic, "manifest") == 0)
            return "{\"version\":\"2.3\",\"topic\":\"extensions\",\"objects\":{\"Salamatrix.extensionManifest\":{\"schema\":2,\"schemaVersionAlias\":\"schemaVersion\",\"compatibleSchemaVersions\":[1,2],\"format\":\"strict UTF-8 JSON\",\"fields\":[\"id\",\"name|title\",\"version\",\"description\",\"runtime\",\"entryPoint\",\"web\",\"security\",\"capabilities\",\"commands\",\"viewers\",\"fileSystems\"],\"runtime\":\"runtime id string or object with id and minimumVersion\",\"entryPoint\":\"safe relative path; absolute and parent traversal are rejected\",\"capabilityValues\":[\"panels.read\",\"panels.write\",\"ui.dialogs\",\"commands\",\"file-operations\",\"file-system\",\"storage\",\"events\",\"ai\",\"clipboard\",\"runtimes\"],\"ungatedSurfaces\":[\"application language and appearance\",\"host metadata\"],\"commandFields\":[\"id\",\"title\",\"handler\",\"path\",\"menu|placement\",\"contextMenu\",\"toolbar\",\"toolbarMenu\",\"requires\"],\"commandPathSemantics\":\"A path-only command changes the active panel directly without starting a runtime worker, requires panels.write, and cannot also declare handler.\",\"viewerFields\":[\"name\",\"patterns\",\"handler\"],\"fileSystemFields\":[\"id\",\"name\",\"listHandler\",\"openHandler\",\"icon\",\"iconDark\",\"refreshIntervalMs\",\"refreshDepth\",\"refreshPaths\",\"columns\",\"actions\"],\"fileSystemColumnFields\":[\"id\",\"name\",\"description\",\"width\",\"numeric\",\"size\",\"dateTime\"],\"fileSystemColumnSizeSemantics\":\"A size column carries an unsigned byte count, is implicitly numeric, and follows Configuration > Panels > Show sizes.\",\"fileSystemColumnDateTimeSemantics\":\"A dateTime column carries Unix milliseconds, sorts by the UTC instant, and displays with the user Windows regional format and time zone.\",\"fileSystemActionFields\":[\"id\",\"title\",\"handler\",\"itemIdPrefix\",\"default\",\"refresh\",\"separator\"],\"fileSystemActionSemantics\":\"Only an explicitly default action runs on Enter. Use separator=true as a standalone entry. Locale resources map fileSystems.<id>.actions.<actionId> to translated titles.\",\"fileSystemMethods\":[\"addItem\",\"addItems\"],\"fileSystemBatchSemantics\":\"Use addItems for a complete listing snapshot; it returns the accepted item count and avoids per-row worker-host round-trips.\",\"requiresValues\":[\"any\",\"disk\",\"focused\",\"file\",\"selection\"],\"generatedPackage\":\"The assistant returns only complete entry-point source. Save as extension creates extension.json and main.<runtime extension>, binds handler main, and copies response capabilities. Do not embed a manifest in script.\"}}}";
