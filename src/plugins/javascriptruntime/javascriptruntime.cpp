...
public:
    static CJavaScriptRuntimeSession* Create(
        HANDLE process,
        HANDLE thread,
        HANDLE input,
        HANDLE output,
        ...
        void* hostDispatchContext)
    {
        return new CJavaScriptRuntimeSession(
            process,
            thread,
            input,
            output,
    ...
    virtual ~CJavaScriptRuntimeSession()
    {
        Stop();
...
CJavaScriptRuntimeAdapter::CJavaScriptRuntimeAdapter(
...
static BOOL ResolveWorkerBootstrapPath(
    CJavaScriptRuntimeAdapter::ProcessKind kind,
    std::wstring* path)
{
    if (path == NULL)
        return FALSE;
    ...
    switch (kind)
    {
    case CJavaScriptRuntimeAdapter::ProcessKindPython:
        fileName = L"salamatrix_worker.py";
        break;
    case CJavaScriptRuntimeAdapter::ProcessKindPowerShell:
        fileName = L"salamatrix_worker.ps1";
        break;
    case CJavaScriptRuntimeAdapter::ProcessKindPhp:
        fileName = L"salamatrix_worker.php";
        break;
    case CJavaScriptRuntimeAdapter::ProcessKindJavaScript:
        fileName = L"salamatrix_worker.mjs";
        break;
    default:
        return FALSE;
    }
...
CJavaScriptRuntimeAdapter::GetDescriptor() const
{
    return &m_oDescriptor;
}

BOOL WINAPI CJavaScriptRuntimeAdapter::IsAvailable() const
{
    ResolveInterpreter();
    ...
    const char* entryPoint) const
{
    ...
    return extension != NULL &&
           (_stricmp(extension, ".js") == 0 ||
            _stricmp(extension, ".mjs") == 0) && IsAvailable();
}

BOOL WINAPI CJavaScriptRuntimeAdapter::Execute(
...
BOOL WINAPI CJavaScriptRuntimeAdapter::StartPersistent(
    const Salamatrix::Runtime::RuntimeExecutionRequest* request,
    Salamatrix::Runtime::IRuntimeSession** session)
{
    ...
    *session = CJavaScriptRuntimeSession::Create(
        process.hProcess,
        process.hThread,
        parentInput,
        parentOutput,
...
static CJavaScriptRuntimeAdapter JavaScriptRuntime(
    "JavaScript.Node",
    "JavaScript Node runtime provider",
    "javascript",
    ".js;.mjs",
    L"SALAMATRIX_NODE",
    L"node",
...
void WINAPI CPluginInterface::About(HWND parent)
{
    ...
    _snprintf_s(text, _countof(text), _TRUNCATE,
        "JavaScript Runtime provider\\n\\nRegistered: %s\\nInterpreter available: %s",
        JavaScriptRegistration.IsRegistered() ? "yes" : "no",
        JavaScriptRuntime.IsAvailable() ? "yes" : "no");
    if (SalamanderGeneral != NULL)
        SalamanderGeneral->SalMessageBox(parent, text, "JavaScript Runtime",
