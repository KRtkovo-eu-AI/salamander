...
class CLuaRuntimeSession : public Salamatrix::Runtime::IRuntimeSession
{
private:
    HANDLE m_hProcess;
    HANDLE m_hThread;
...
public:
    static CLuaRuntimeSession* Create(
        HANDLE process,
        HANDLE thread,
        HANDLE input,
        HANDLE output,
...
CLuaRuntimeAdapter::CLuaRuntimeAdapter(
    ...
    : m_pszExtension(fileExtension),
      ...
      m_pszCandidateThree(candidateThree),
      m_kind(kind),
...
static BOOL ResolveWorkerBootstrapPath(
    CLuaRuntimeAdapter::ProcessKind kind,
    std::wstring* path)
{
    if (path == NULL)
        return FALSE;
    ...
    switch (kind)
    {
    case CLuaRuntimeAdapter::ProcessKindPython:
        fileName = L"salamatrix_worker.py";
        break;
    case CLuaRuntimeAdapter::ProcessKindLua:
        fileName = L"salamatrix_worker.lua";
        break;
    case CLuaRuntimeAdapter::ProcessKindPhp:
...
void CLuaRuntimeAdapter::ResolveInterpreter() const
{
...
CLuaRuntimeAdapter::GetDescriptor() const
{
    return &m_oDescriptor;
}

BOOL WINAPI CLuaRuntimeAdapter::IsAvailable() const
{
    ResolveInterpreter();
    return m_executablePath.empty() ? FALSE : TRUE;
}

const std::wstring& CLuaRuntimeAdapter::GetExecutablePath() const
{
    ResolveInterpreter();
    return m_executablePath;
}

void CLuaRuntimeAdapter::InvalidateExecutablePath()
{
    m_executablePath.clear();
    m_bInterpreterResolved = false;
}

BOOL WINAPI CLuaRuntimeAdapter::SupportsEntryPoint(
...
BOOL WINAPI CLuaRuntimeAdapter::Execute(
...
BOOL WINAPI CLuaRuntimeAdapter::StartPersistent(
    const Salamatrix::Runtime::RuntimeExecutionRequest* request,
    Salamatrix::Runtime::IRuntimeSession** session)
{
    ...
    *session = CLuaRuntimeSession::Create(
        process.hProcess,
        process.hThread,
        parentInput,
        parentOutput,
...
static CLuaRuntimeAdapter LuaRuntime(
    "Lua",
    "Lua runtime provider",
    "lua",
    ".lua",
    L"SALAMATRIX_LUA",
    L"lua.exe",
    L"lua55.exe",
    L"lua54.exe",
    CLuaRuntimeAdapter::ProcessKindLua);
static Salamatrix::Runtime::RuntimeProviderRegistration LuaRegistration;
...
static BOOL TryRegisterLuaRuntime()
{
    if (LuaRegistration.IsRegistered())
        return TRUE;
    if (SalamanderGeneral == NULL)
        return FALSE;
    CSalamanderServiceQuery query;
    ...
    Salamatrix::Runtime::IRuntimeService* runtime =
        static_cast<Salamatrix::Runtime::IRuntimeService*>(
            serviceResult.Interface);
    if (runtime->FindAdapter("Lua", 0) != NULL)
        return TRUE;
    return LuaRegistration.Register(runtime, &LuaRuntime)
               ? TRUE
               : FALSE;
}
...
void WINAPI CPluginInterface::About(HWND parent)
{
    ...
    _snprintf_s(text, _countof(text), _TRUNCATE,
        ...
        LuaRegistration.IsRegistered() ? "yes" : "no",
        LuaRuntime.IsAvailable() ? "yes" : "no");
    if (SalamanderGeneral != NULL)
        SalamanderGeneral->SalMessageBox(parent, text, "Lua Runtime",
...
void WINAPI CPluginInterface::Configuration(HWND parent)
{
    ...
    LuaRuntime.InvalidateExecutablePath();
    std::wstring automaticPath = LuaRuntime.GetExecutablePath();
    RuntimeSettings.UseCustomExecutable = useCustom;
    LuaRuntime.InvalidateExecutablePath();
    std::wstring effectivePath = LuaRuntime.GetExecutablePath();
...
{
    UnregisterRuntimeProvider(LuaRegistration);
