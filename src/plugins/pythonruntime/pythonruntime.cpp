...
public:
    static CPythonRuntimeSession* Create(
        HANDLE process,
        HANDLE thread,
        HANDLE input,
        HANDLE output,
        ...
        void* hostDispatchContext)
    {
        return new CPythonRuntimeSession(
            process,
            thread,
            input,
            output,
...
static BOOL ResolveWorkerBootstrapPath(
    CPythonRuntimeAdapter::ProcessKind kind,
    std::wstring* path)
{
    if (path == NULL)
        return FALSE;
    ...
    switch (kind)
    {
    case CPythonRuntimeAdapter::ProcessKindPython:
        fileName = L"salamatrix_worker.py";
        break;
    case CPythonRuntimeAdapter::ProcessKindPowerShell:
        fileName = L"salamatrix_worker.ps1";
        break;
    case CPythonRuntimeAdapter::ProcessKindPhp:
...
void CPythonRuntimeAdapter::ResolveInterpreter() const
{
...
CPythonRuntimeAdapter::GetDescriptor() const
{
    return &m_oDescriptor;
}

BOOL WINAPI CPythonRuntimeAdapter::IsAvailable() const
{
    ResolveInterpreter();
...
BOOL WINAPI CPythonRuntimeAdapter::Execute(
...
BOOL WINAPI CPythonRuntimeAdapter::StartPersistent(
    const Salamatrix::Runtime::RuntimeExecutionRequest* request,
    Salamatrix::Runtime::IRuntimeSession** session)
{
    ...
    *session = CPythonRuntimeSession::Create(
        process.hProcess,
        process.hThread,
        parentInput,
        parentOutput,
...
static CPythonRuntimeAdapter PythonRuntime(
    "Python.CPython",
    "CPython runtime provider",
    "python",
    ".py",
    L"SALAMATRIX_PYTHON",
    L"python.exe",
    L"python3.exe",
    CPythonRuntimeAdapter::ProcessKindPython);
static Salamatrix::Runtime::RuntimeProviderRegistration PythonRegistration;

static BOOL TryRegisterPythonRuntime()
{
    if (PythonRegistration.IsRegistered())
        return TRUE;
    ...
    if (runtime->FindAdapter("Python.CPython", 0) != NULL)
        return TRUE;
    return PythonRegistration.Register(runtime, &PythonRuntime) ? TRUE : FALSE;
}
