...
const std::wstring& CPHPRuntimeAdapter::GetExecutablePath() const
{
    ResolveInterpreter();
    return m_executablePath;
}

void CPHPRuntimeAdapter::InvalidateExecutablePath()
{
    m_executablePath.clear();
    m_bInterpreterResolved = false;
}
...
void WINAPI CPluginInterface::Configuration(HWND parent)
{
    ...
    PHPRuntime.InvalidateExecutablePath();
    std::wstring automaticPath = PHPRuntime.GetExecutablePath();
    RuntimeSettings.UseCustomExecutable = useCustom;
    PHPRuntime.InvalidateExecutablePath();
    std::wstring effectivePath = PHPRuntime.GetExecutablePath();
