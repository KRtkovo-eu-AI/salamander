...
foreach($language in $selectedLanguages){foreach($module in $selectedModules){
 try {
  ...
  if(-not $ImportOnly){
   ...
   $args=@($rebased,$rebased,'--language',$language,'--model',$Model,'--batch-size',[Math]::Max(1,40*$MaxParallelRequests),'--trace-file',$apiTrace); if($DryRun){$args+='--dry-run'}; if($ForceRetranslate -or $translateAllCurrentStrings){$args+='--force-retranslate'}
