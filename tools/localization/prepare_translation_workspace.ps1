...
if (-not (Test-Path -LiteralPath (Join-Path $buildRootPath "salamand.exe")))
{
    throw "Build root '$buildRootPath' does not look like a populated Samandarin output directory."
}
...
foreach ($language in $requestedLanguages)
{
    foreach ($module in $selectedModules)
    {
        ...
        New-ProjectFile `
            ...
            -SalamanderExePath (Join-Path $runtimeRoot "salamand.exe") `
